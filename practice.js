...
function removeItem (grocArrR, item) {
  if(!grocArrR || !item) return [];
  for (i=0;i<grocArrR.length;i++) {
    if (item === grocArrR[i]) {
      grocArrR.splice(i, 1);
    }
  }
...
function addItem (grocArr, item) {
  if (!grocArr || !item) return [];
  grocArr.push(item);
  return grocArr;
};
...
function longer(arrIn1, arrIn2) {
  if (arrIn1.length > arrIn2.length) {
    return arrIn1;
  } else {
    return arrIn2;
  }
...
function both (arrIn1, arrIn2) {
  var newArray = [];
  for (i=0;i<arrIn1.length;i++) {
    if (arrIn2.includes(arrIn1[i])) {
      newArray.push(arrIn1[i]);
    }
  }
  return newArray;
};
...
var user3 = {
  ...
  password: 'imlostbutnotalone',
  username: 'FindMeandMe'
};

var user4 = {
  name: 'Dog the Bounty Hunter',
  email: 'BigDog@gmail.com',
  password: 'icanfindanyone',
  username: 'BigTracker'
};
...
removeUser(users, "Tyler McGinnis");
